...
protected:
	...
	shared_ptr<DataLoader> _create_remote_loader(string const& host, int& port);
	shared_ptr<DataLoader> _create_local_loader(string const& datasetName, string const& userFile,
			string const& itemFile, string const& ratingFile);
public:
	/**
	 * @brief get a data loader based on the arguments
	 */
	shared_ptr<DataLoader> get_loader(int argc, char** argv);
	shared_ptr<DataLoader> get_remote_loader(int argc, char** argv);
	shared_ptr<DataLoader> get_local_loader(int argc, char** argv);
	shared_ptr<DataLoader> get_remote_loader(string const& host, int port);
	shared_ptr<DataLoader> get_local_loader(string const& datasetName, string const& userFile, string const& itemFile, string const& ratingFile);
