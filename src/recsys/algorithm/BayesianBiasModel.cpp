...
void BayesianBiasModel::dump_model_text(string const& filePrefix){
	...
	string priorFile = filePrefix + ".prior.txt";
	ofs.open(priorFile.c_str(), std::ofstream::out);
	assert(ofs.good());

	float globalBiasMean = (float)m_global_bias.moment(1);
	ofs  << globalBiasMean << "," << (float)m_global_bias.moment(2) - globalBiasMean * globalBiasMean << "\n";
	ofs  << m_user_bias_mean_prior.moment(1) << "," << m_user_bias_var_prior.moment(1) << "\n";
	ofs  << m_item_bias_mean_prior.moment(1) << "," << m_item_bias_var_prior.moment(1) << "\n";
	ofs.close();

}

vector<rt::Recommendation> BayesianBiasModel::recommend(int64_t const& userId, map<int8_t, vector<rt::Interact> >& userInteracts){
	...
	if (userIds.find(userId) == userIds.end()) {
		...
		_update_user_bias_from_ratings(userId, userInteracts[EntityInteraction::RATE_ITEM]);
	}
...
void BayesianBiasModel::_update_global_bias() {
	...
	for (set<int64_t>::iterator iter = userIds.begin(); iter != userIds.end(); ++iter) {
		...
		for (vector<Interact>::const_iterator iter1 = ratings.begin(); iter1
			...
			updateMessage[0].m_vec += (rating - userBias
					- itemBias);
			numRatings++;
		}
	}
...
void BayesianBiasModel::_update_user_bias_from_ratings(int64_t const& userId,	vector<Interact>& ratingInteracts) {
	float rvSuff2 = (float) m_rating_var.suff_mean(2);
	DistParamBundle updateMessage(2);
	updateMessage[0] = updateMessage[1] = (float) 0;
	...
	for (vector<Interact>::const_iterator iter1 = ratingInteracts.begin(); iter1
		...
		updateMessage[0].m_vec += (rating  - m_item_bias[itemId].moment(1) - m_global_bias.moment(1));
	}
	updateMessage[0].m_vec *= (rvSuff2);
	updateMessage[1].m_vec = -0.5 * numRatings * rvSuff2;
	/// update the bias
...
void BayesianBiasModel::_update_item_bias_from_ratings(int64_t const& itemId, vector<Interact>& ratingInteracts) {
	float rvSuff2 = (float) m_rating_var.suff_mean(2);
	DistParamBundle updateMessage(2);
	updateMessage[0] = updateMessage[1] = (float) 0;
	size_t numRatings = ratingInteracts.size();
	for (vector<Interact>::const_iterator iter1 = ratingInteracts.begin(); iter1
		...
		updateMessage[0].m_vec += (rating  - m_user_bias[userId].moment(1) - m_global_bias.moment(1));
	}
	updateMessage[0].m_vec *= (rvSuff2);
	updateMessage[1].m_vec = -0.5 * numRatings * rvSuff2;
	/// update the bias
...
void BayesianBiasModel::_update_user_bias(int64_t const& userId, map<int8_t, vector<
...
void BayesianBiasModel::_update_item_bias(int64_t const& itemId, map<int8_t, vector<
		Interact> > & typeInteracts) {
...
float BayesianBiasModel::_pred_error(int64_t const& userId, DatasetExt& dataset){
	float rmse = 0;
	...
	for (vector<Interact>::iterator iter = ratingInteracts.begin();
		...
		if (m_active_dataset.ent_ids.find(itemId)
			...
			m_item_bias[itemId].reset();
			_update_item_bias_from_prior(itemId,dataset.ent_type_interacts[itemId][EntityInteraction::ADD_FEATURE]);
		}
	}
	...
	for (vector<Interact>::iterator iter = ratingInteracts.begin();
		...
		int64_t itemId = iter->ent_id;
		float predRating =  (float) m_global_bias.moment(1) + (float)m_user_bias[userId].moment(1) + (float)m_item_bias[itemId].moment(1);
...
void BayesianBiasModel::_add_new_entity(int64_t const& entityId, int8_t const& entityType){
