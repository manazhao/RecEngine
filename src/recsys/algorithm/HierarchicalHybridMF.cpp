...
float HierarchicalHybridMF::_pred_error(int64_t const& userId, map<int8_t,
		vector<Interact> >& entityInteractMap) {
	...
	for (vector<Interact>::iterator iter = featureInteracts.begin(); iter
			< featureInteracts.end(); ++iter) {
		int64_t entityId = iter->ent_id;
...
//		_update_entity_from_ratings(userId, userInteracts);
	...
	for(set<int64_t>::iterator iter = itemIds.begin(); iter != itemIds.end(); ++iter){
		...
		rec.score = accu(itemLat.moment(1).m_vec
				% userLat.moment(1).m_vec) + (float) m_bias.moment(1);
